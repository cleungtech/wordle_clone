...
const Game = () => {
  ...
  const checkGameState = () => {
    ...
    const isCorrectAnswer = previousRow.every((letter, i) => letter === letters[i]);
   ...
   */
  const shareScore = () => {
    const textMap = rows.map((row, i) => row.map((cell, j) => colorsToEmoji[getCellBackGroundColor(i, j)]).join("")
    )
      .filter((row) => row)
      .join("\n");
    const textShare = `Wordle ${currentRow}/${NUMBER_OF_TRIES}\n\n${textMap}`;
    Clipboard.setString(textShare);
    Alert.alert(`${textShare}\n\nCopied!\nPaste anywhere to share!`);
  };
  ...
  const getAllLettersWithColor = (color) => {
    ...
    return rows.flatMap((row, i) => row.filter((cell, j) => {
